...
test.describe('Basic Usage Flow', () => {
  ...
  test('dismisses suggestion on Escape', async ({ page }) => {
    ...
    const ghost = page.locator('div[style*="z-index: 9999"]').filter({ hasText: 'bug report' });
    await expect(ghost).toBeVisible();
  ...
  test('clears suggestion on blur', async ({ page }) => {
    ...
    const ghost = page.locator('div[style*="z-index: 9999"]').filter({ hasText: 'deploy to prod' });
    await expect(ghost).toBeVisible();
